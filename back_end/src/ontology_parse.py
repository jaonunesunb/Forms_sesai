...
def extract_labels(g):
    ...
    def get_label(entity):
        for label in g.objects(entity, RDFS.label):
            if label.language and label.language.startswith('pt'):
                return str(label)
        for label in g.objects(entity, RDFS.label):
            return str(label)
        return None
...
def process_restriction(g, restriction, labels):
    ...
    exact_cardinality = g.value(restriction, OWL.qualifiedCardinality)
    ...
    if min_cardinality is not None:
        ...
        cardinality_str = f"exactly {exact_cardinality}"  # Corrigido para lidar com cardinalidade exata
...
def list_restrictions_and_data_properties(g, class_uri, labels, labels_to_uris):
    ...
    for restriction in restrictions:
        ...
        print(f"  Propriedade: {property_label}, Classe: {related_class_labels}, Cardinalidade: {cardinality}")
        ...
        for related_class in flattened_related_classes:
            ...
            if isinstance(related_class, URIRef):
                ...
                if data_props:
                    print(f"    Tipos de Dados Associados à Classe {labels.get(related_class, related_class)}:")
                    for prop_uri, prop_restrictions in data_props:
                        prop_label = labels.get(prop_uri, prop_uri)
                        print(f"      Propriedade: {prop_label}, Restrições: {prop_restrictions}")
...
def main():
    ...
    print(f"Subclasses de {labels.get(URIRef(class_uri), class_uri)}:")
    for subclass in subclasses:
        print(f"  - {subclass}")
    
    # Listar restrições e data properties associadas
    print(f"Restrições da classe {labels.get(URIRef(class_uri), class_uri)}:")
